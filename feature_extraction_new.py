...
################################################################
# new argument: for patch files instead of wsi
################################################################

parser.add_argument(
    "--patch_imgs",
    help="True when patch images are available, skips patching of WSI",
    action='store_true',
)
...
def main(args):
    ...
    ################################################################################################################
    # if else based on patch_imgs
    ################################################################################################################

    if args.patch_imgs:
        extract_features_from_patches(slide_files, model_dicts, device, args)
    else:
...
####################################################################################
# new function for patch images instead of wsi, saves .h5 per class
####################################################################################
...
def extract_features_from_patches(
    ...
    """
    Extract features from patch image files using a given model.

    Args:
        file_paths (list): list of all image (patches) file paths.
        args (argparse.Namespace): Arguments containing various processing parameters.
        model_dict (dict): Dictionary containing the model, transforms, and model name.
        device (torch.device): Device to perform computations on (CPU or GPU).
...
##############################################################################################
# new feature extractor with better runtime,
# since original functions works optimally for wsi not patches
##############################################################################################

def patch_files_to_feature(
    img_paths: list, patch_class: str, model_dicts: list[dict], device: torch.device, histaugan=None,
):
    """
    predict features for a given patch image list

    Args:
        img_paths (list): list of all image (patches) file paths.
        patch_class (str): the class of patch images it originates from.
        model_dict (dict): Dictionary containing the model, transforms, and model name.
        device (torch.device): Device to perform computations on (CPU or GPU).
        histaugan (bool): enables patch-level augmentations
    ...
    with torch.no_grad():
        for model_dict in model_dicts:
            ...
            data_l = tqdm(dataloader, total=len(dataloader), unit='batch')
            for batch in data_l:
                batch = batch.to(device)
